...
class AbstractBlackBoxAlgorithm(object):
    ...
    """
    def __init__(self, n, m, func=None):
        """
        ...
        from 0 to 2^m - 1, inclusive. If it is initialized to None, the assumption is that the default oracle generation is overwritten.
    ...
    def _generate_prog(self):
        ...
        by appending to self._prog. Uses a specific, algorithm-specific method to generate a program
        based off an algorithm specific oracle.
        :return: None
        """
    ...
    def generate_oracle(self):
        """
        Generates the program corresponding to the oracle. Can be overridden for greater efficiency, dependinng on
        the properties of the specific algorithm.

        By default, the general progression is:
        from the function properties, generate a unitary that properly acts as the oracle,
        ...
        :return: The program corresponding to the oracle.
        :rtype: Program
        """
        if self._func is None:
            raise NotImplementedError("func not given; generate_oracle must be overridden.")
